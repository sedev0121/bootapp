...
public interface NegotiationDetailRepository extends JpaRepository<NegotiationDetail, Long> {
	...
	@Query(value = "SELECT 0 id, :code code, tax_rate, inventory_code, price, 100 max_quantity, tax_price, now() start_date, DATE_ADD(now(), interval 3 month) end_date, '' memo, 0 valid FROM purchase_order_detail WHERE code=:orderCode", nativeQuery = true)
