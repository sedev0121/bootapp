...
public interface PurchaseOrderMainRepository extends JpaRepository<PurchaseOrderMain, Long> {
	...
	@Query(value = "SELECT a.*, b.name vendorname, c.realname deployername, d.realname reviewername FROM purchase_order_main a left join vendor b on a.vencode=b.code left join account c on a.deployer=c.id left join account d on a.reviewer=d.id WHERE a.state='审核' and a.code like %?1% and (b.code LIKE %?2% or b.name LIKE %?2% )", countQuery = "SELECT count(*) FROM purchase_order_main a left join vendor b on a.vencode=b.code WHERE a.code like %?1% and (b.code LIKE %?2% or b.name LIKE %?2% )", nativeQuery = true)
