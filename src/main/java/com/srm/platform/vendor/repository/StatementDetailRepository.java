...
public interface StatementDetailRepository extends JpaRepository<StatementDetail, Long> {
	...
	@Query(value = "select a.*, d.code pi_code, c.auto_id pi_auto_id, c.state pi_state, c.erp_changed pi_erp_changed, c.tax_rate pi_tax_rate, d.vendor_code pi_vendor_code, d.store_code pi_store_code, d.date pi_date, e.code inventory_code, e.name inventory_name, e.specs, e.main_measure unitname, "
			+ "c.quantity pi_quantity, pom.code po_code, po.row_no po_row_no, c.tax pi_tax, c.price pi_price, c.tax_price pi_tax_price, c.cost pi_cost, c.tax_cost pi_tax_cost, po.confirmed_memo confirmed_memo, "
	...
	@Query(value = "select b.vendor_code, b.type, c.statement_company_id from purchase_in_detail a "
			+ "left join purchase_in_main b on a.main_id=b.id left join company c on b.company_code=c.code "
			+ "where a.state=0 and a.erp_state=1 and (a.bill_quantity is null or a.bill_quantity =0) and b.company_code is not null and b.date < ?1 and b.vendor_code in (select vendor_code from account where role='ROLE_VENDOR' and vendor_code is not null) GROUP BY b.vendor_code, b.type, c.statement_company_id", nativeQuery = true)
	List<StatementPendingItem> findAllPendingData(Date filterDate);
	
	@Query(value = "select a.id, b.code from purchase_in_detail a left join purchase_in_main b on a.main_id=b.id left join company c on b.company_code=c.code left join statement_company d on c.statement_company_id=d.id "
			+ "where a.state=0 and a.erp_state=1 and (a.bill_quantity is null or a.bill_quantity =0) and b.company_code is not null and b.vendor_code = ?1 and d.id = ?2 and b.type = ?3 and b.date < ?4 order by b.code, a.row_no", nativeQuery = true)
