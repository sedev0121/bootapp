...
public interface StatementDetailRepository extends JpaRepository<StatementDetail, Long> {
	...
	@Query(value = "select a.*, c.quantity-ifnull(a.closed_quantity, 0) remain_quantity, c.code purchase_in_code, c.rowno, c.quantity, c.price, c.cost, c.tax_price, c.tax_cost, c.nat_tax_price, c.material_quantity, c.material_tax_price, "
			...
			+ "left join measurement_unit f on e.main_measure=f.code where a.code=?1 and a.purchasein_type=?2 order by c.code, c.rowno", nativeQuery = true)
	List<StatementDetailItem> findDetailsByCode(String code, Integer purchaseInType);
